...
def _create(parms):
    result = {'status': 'ok'}
    grids = {
        1: [-8, -1, -5, -7, -6, -9, -3, -2, 0, -4, -9, 0, 0, 0, -5, -8, -7, 0, 0, 0, -6, 0, -4, -8, 0, -9, -5, 0, -8, -1, 0, 0, -3, 0, 0, -2, 0, -5, 0, -1, -8, 0, -9, 0, -7, -7, -3, -9, -5, -2, -4, -6, -8, -1, -9, -4, 0, 0, 0, -7, 0, -1, -8, -5, -2, 0, -8, -9, 0, -4, -6, -3, -1, -6, 0, -4, -3, -2, -7, 0, 0],
    ...
    if 'level' in parms:
        level = int(parms['level'])
        if level in grids:
            result['grid'] = grids[level]
            result['integrity'] = hashlib.sha256(json.dumps(result['grid']).encode()).hexdigest()
        else:
            result['status'] = 'hata: hatalı değer girdiniz'
    else:
        # If level is not provided, set it to 3
        level = 3
        result['grid'] = grids[level]
        result['integrity'] = hashlib.sha256(json.dumps(result['grid']).encode()).hexdigest()
